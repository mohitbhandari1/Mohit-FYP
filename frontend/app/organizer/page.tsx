...
export default function OrganizerDashboard() {
  ...
  // Pending registration requests (RSVPs awaiting approval)
  ...
  // Load requests when the Requests tab is opened
  useEffect(() => {
  ...
  const handleRsvpRequest = async (req: PendingRsvpRequest, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (action === 'approve') {
          // Reflect the new attendee in the events list counters
          setMyEvents((prev) => prev.map((e: any) =>
            e.id === req.event_id
              ? { ...e, attendee_count: (e.attendee_count || 0) + 1, seats_remaining: e.seats_remaining != null ? Math.max(e.seats_remaining - 1, 0) : null }
              : e
          ));
        }
